...
export const trackRemainingBlocks = async () => {
  ...
  if (currentNumber !== null) {
    ...
    if (currentNumber === 864) {
      console.log("Do something when number becomes equal to or lower than 864");
      await addAmount(); // Reset NFT collections
    } else if (currentNumber === 1728) {
      console.log("Do something when number becomes equal to or lower than 1,728");
      await addAmount(); // Reset NFT collections
    } else if (currentNumber === 2592) {
      console.log("Do something when number becomes equal to or lower than 2,592");
      await addAmount(); // Reset NFT collections
    } else if (currentNumber === 3456) {
      console.log("Do something when number becomes equal to or lower than 3,456");
      await addAmount(); // Reset NFT collections
    } else if (currentNumber === 4320) {
      console.log("Do something when number becomes equal to or lower than 4,320");
      await addAmount(); // Reset NFT collections
    } else {
